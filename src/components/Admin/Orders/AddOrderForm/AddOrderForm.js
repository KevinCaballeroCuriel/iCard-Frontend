...
export function AddOrderForm(props) {
  const { idTable, openCloseModal, onRefetch } = props;
  ...
  const formik = useFormik({
    ...
    onSubmit: async (formValue) => {
      for await (const idProduct of formValue.products) {
        await addOrder(idTable, idProduct);
      }
  ...
  return (
    <Form className="add-order-form" onSubmit={formik.handleSubmit}>
      <Dropdown
        ...
        noResultsMessage="No se encuetra este producto"
        onChange={(_, data) =>
      ...
      <Button
        ...
        primary
        fluid
        content="Agregar productos a la mesa"
      />
    </Form>
  );
}
